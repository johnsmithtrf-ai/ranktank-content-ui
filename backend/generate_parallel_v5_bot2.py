...
# ─── v5: PAA / Related-Questions, Anchor Diversity, E-E-A-T, Self-Check ─────────
SCRIPT_VERSION = "v5"
AHREFS_TOKEN = "2hHH9gkDW15WZSzdHvWh8jlWujY15glYymvIttvN"

# PAA cache TTL (12h) — separate from SERP cache
PAA_CACHE_TTL = 43200

async def fetch_paa_questions(keyword: str, geo: str, client, limit: int = 20) -> list:
    """
    v5: Fetch People Also Ask questions from Ahrefs keywords-explorer/related-questions.
    Returns list of up to `limit` question strings.
    Falls back to empty list if unavailable (prompt uses generic FAQ then).
    """
    import json as _jc, time as _tc, re as _re

    geo_map = {
        "uk": "gb", "united kingdom": "gb", "great britain": "gb",
        "us": "us", "usa": "us", "united states": "us",
        "de": "de", "germany": "de",
        "nl": "nl", "it": "it", "pt": "pt", "br": "br",
        "ca": "ca", "au": "au", "es": "es", "fr": "fr",
    }
    geo_code = geo_map.get(geo.lower().strip(), geo.lower()[:2])

    _cache_key = _re.sub(r"[^a-z0-9_]", "_", f"paa_{keyword}_{geo_code}".lower())
    _cache_path = f"/tmp/ahrefs_paa_{_cache_key}.json"

    # ─ Cache check ─
    if os.path.exists(_cache_path):
        try:
            _cd = _jc.loads(open(_cache_path).read())
            if _tc.time() - _cd.get("ts", 0) < PAA_CACHE_TTL:
                questions = _cd.get("questions", [])
                print(f"  ✅ PAA cache HIT: {len(questions)} questions for '{keyword}'")
    ...
    questions = []
    try:
        # Ahrefs v3 keywords-explorer related-questions endpoint
        url = (
            f"https://api.ahrefs.com/v3/keywords-explorer/related-questions"
            f"?select=keyword&keyword={keyword}&country={geo_code}&limit={limit}"
        )
        r = await client.get(
            url,
        ...
        if r.status_code == 200:
            ...
            raw = data.get("keywords", data.get("related_questions", []))
            questions = [item["keyword"] for item in raw if item.get("keyword")][:limit]
            print(f"  ✅ PAA: {len(questions)} questions fetched for '{keyword}' / {geo_code.upper()}")
        else:
            print(f"  ⚠️ PAA Ahrefs HTTP {r.status_code}: {r.text[:120]}")
    except Exception as e:
        print(f"  ⚠️ PAA fetch error: {e}")

    # Save cache
    if questions:
        try:
            open(_cache_path, "w").write(_jc.dumps({"ts": _tc.time(), "questions": questions}))
        except Exception:
            pass
...
def build_paa_prompt_block(paa_questions: list, lang: str) -> str:
    """
    v5: Build prompt injection block from PAA questions.
    Instructs AI to use top-7 as H3 subheadings + end FAQ block.
    """
    if not paa_questions:
        return ""

    top7 = paa_questions[:7]
    all_q = paa_questions[:20]

    lines = [
        "\n═══ 💬 PAA (PEOPLE ALSO ASK) — REAL GOOGLE QUESTIONS ═══",
        f"⚠️ These are REAL questions users ask Google about this topic.",
        f"You MUST answer them in the article — do NOT invent your own FAQ questions.",
        "",
        f"▸ USE TOP-7 AS H3 SUBHEADINGS inside relevant H2 sections (translate to {lang} if needed):",
    ]
    for i, q in enumerate(top7, 1):
        lines.append(f"  {i}. {q}")

    lines += [
        "",
        f"▸ END-OF-ARTICLE FAQ BLOCK — answer ALL {len(all_q)} questions below:",
        f"  Format: [FAQ QUESTION: <question in {lang}>] / [FAQ ANSWER: 3-5 sentences, direct answer first]",
        f"  Each answer MUST start with a direct 1-sentence answer (featured snippet format).",
        f"  Questions:",
    ]
    ...
    lines += [
        "",
        "⚠️ FAQPage schema will be built from these questions only — do NOT add unrelated FAQ.",
        "═══════════════════════════════════════════════",
    ]
    return "\n".join(lines) + "\n"


ANCHOR_DIVERSITY_RULES = """
═══ 🔗 ANCHOR TEXT DIVERSITY (v5 rule — MANDATORY) ═══
Every internal link must use a VARIED anchor. Distribute by these proportions:
  • Exact match (primary KW exact)         10%   e.g. "best online casino"
  • Partial match (KW + extra words)        20%   e.g. "top online casinos UK"
  • Variation (synonym / related KW)        20%   e.g. "best gambling sites"
  • Related/topical (broader context)       20%   e.g. "online gambling guide"
  • Branded (domain/brand name)             15%   e.g. site brand name
  • Generic (natural language)              10%   e.g. "read more", "this guide", "click here"
  • LSI/long-tail (3-5 word specific)        5%   e.g. "licensed casinos with fast payouts"
⚠️ Do NOT repeat the same anchor text for two different links.
⚠️ Avoid all-exact-match anchors — Google spam-filters detect anchor patterns.
═══════════════════════════════════════════════
"""
...
def strip_technical_blocks(text: str) -> str:
    """Удаляет технические блоки [SELF_CHECK_REPORT] и [SCHEMA_RECOMMENDATIONS] из готового текста.
    Эти блоки генерируются AI по инструкции в промпте и не должны попадать в финальный файл.
    """
    import re as _re_strip
    # Удаляем всё от [SCHEMA_RECOMMENDATIONS] до конца или до [SELF_CHECK_REPORT]
    # и сам [SELF_CHECK_REPORT] блок
    # Паттерны: тег на отдельной строке (plain text, HTML <p>, или просто текст)
    _patterns = [
        # HTML-обёрнутые теги: <p>[SCHEMA_RECOMMENDATIONS]</p> и всё после
        r'<p>\[SCHEMA_RECOMMENDATIONS\]</p>.*',
        # HTML-обёрнутые теги: <p>[SELF_CHECK_REPORT]</p> и всё после
        r'<p>\[SELF_CHECK_REPORT\]</p>.*',
    ...
    for pat in _patterns:
        text = _re_strip.sub(pat, '', text, flags=_re_strip.DOTALL | _re_strip.IGNORECASE)
    # Также стриповать отдельные строки с метриками типа WORD_COUNT: / FLESCH_SCORE: если вдруг остались
    _line_patterns = [
        r'^WORD_COUNT:.*$',
        r'^FLESCH_SCORE:.*$',
        r'^AVG_SENTENCE_LEN:.*$',
        r'^AVG_PARAGRAPH_SENTENCES:.*$',
        r'^WALL_OF_TEXT_FLAGS:.*$',
        r'^KEYWORD_STUFFING:.*$',
        r'^HAS_FIRST_HAND_EXPERIENCE:.*$',
        r'^HAS_INVENTED_FACTS:.*$',
        r'^RESPONSIBLE_GAMBLING_BLOCK:.*$',
        r'^ANCHOR_DISTRIBUTION:.*$',
        r'^FAQ_FROM_PAA:.*$',
        r'^SCHEMA_TYPES:.*$',
        r'^<p>WORD_COUNT:.*</p>$',
        r'^<p>FLESCH_SCORE:.*</p>$',
        r'^<p>AVG_SENTENCE_LEN:.*</p>$',
        r'^<p>AVG_PARAGRAPH_SENTENCES:.*</p>$',
        r'^<p>WALL_OF_TEXT_FLAGS:.*</p>$',
        r'^<p>KEYWORD_STUFFING:.*</p>$',
        r'^<p>HAS_FIRST_HAND_EXPERIENCE:.*</p>$',
        r'^<p>HAS_INVENTED_FACTS:.*</p>$',
        r'^<p>RESPONSIBLE_GAMBLING_BLOCK:.*</p>$',
        r'^<p>ANCHOR_DISTRIBUTION:.*</p>$',
        r'^<p>FAQ_FROM_PAA:.*</p>$',
        r'^<p>SCHEMA_TYPES:.*</p>$',
    ]
    lines = text.split('\n')
    ...
    for line in lines:
        ...
        for lp in _line_patterns:
            if _re_strip.match(lp, line.strip(), flags=_re_strip.IGNORECASE):
                skip = True
                break
        if not skip:
            cleaned.append(line)
...
def build_prompt(task, article_type, casino=None, text_index=0, serp_targets=None, pre_title=None):
    ...
    base_rules = tf_rule + struct_rule + deep_rule + aggressive_rule + (
        ...
        # ── v5: Lemmatization rule ──
        f"▸ KEYWORD LEMMATIZATION (v5 — MANDATORY for non-EN languages):\n"
        f"  Use the PRIMARY keyword and ALL its natural inflected/declined forms — do NOT repeat only the exact form.\n"
        f"  Language {lang} rules:\n"
        + (
            f"    DE: decline freely (das Casino / dem Casino / die Casinos / eines Casinos)\n"
            f"    All forms of the keyword count as 1 lemma — DO NOT force exact-match only.\n"
            if lang in ('DE', 'de') else
            f"    IT: use all forms (casino / casin\u00f2 / casin\u00f2 esteri / casin\u00f2 online)\n"
            if lang in ('IT', 'it') else
            f"    ES: use all forms (casino / casinos / casino online / casinos online)\n"
            if lang in ('ES', 'es') else
            f"    FR: use all forms (casino / casinos / casino en ligne)\n"
            if lang in ('FR', 'fr') else
            f"    PT/BR: use all forms (casino / casinos / casino online)\n"
            if lang in ('PT', 'pt', 'BR', 'br') else
            f"    Use all natural grammatical forms of the keyword.\n"
        )
        + f"  NEVER force exact-match keyword into unnatural positions.\n"
          f"  Goal: natural-sounding text where all keyword forms collectively hit density target.\n\n"
    )
    ...
    _paa_questions = []
    if serp_targets:
        _paa_questions = serp_targets.get("paa_questions", [])
    # Also allow PAA passed directly in task (manual override)
    if not _paa_questions:
        _paa_questions = task.get("paa_questions", [])
    _paa_block = build_paa_prompt_block(_paa_questions, lang)

    # ── v5: Responsible gambling + compliance block ──
    _responsible_block = (
        f"═══ 🛡️ RESPONSIBLE GAMBLING + COMPLIANCE (v5 — MANDATORY OUTPUT BLOCK) ═══\n"
        f"EVERY article MUST end with a dedicated Responsible Gambling section (H2 in {lang}).\n"
        f"Include:\n"
        f"  • Age restriction for {geo} market (18+/21+ depending on jurisdiction)\n"
        f"  • Risk warning: gambling can be addictive, never play with money you cannot afford to lose\n"
        f"  • Link placeholder to local help: [RESPONSIBLE_GAMBLING_LINK_{geo.upper()}]\n"
        f"  • Informative, neutral tone — NO aggressive marketing, NO guaranteed-win promises\n"
        f"  • Legal disclaimer adapted for {geo} jurisdiction\n"
        f"⚠️ Do NOT use persuasive/manipulative language (\"Win big now!\", \"Exclusive deal!\") — YMYL compliance.\n"
        f"═══════════════════════════════════════════════\n"
    )

    # ── v5: Schema recommendation block ──
    _schema_block = (
        f"═══ 📝 SCHEMA MARKUP RECOMMENDATIONS (v5) ═══\n"
        f"After the article, add a section titled [SCHEMA_RECOMMENDATIONS]:\n"
        f"  • FAQPage schema — built from the FAQ block questions ONLY (not invented)\n"
        f"  • Article / NewsArticle — with datePublished, dateModified, author placeholder\n"
        f"  • If article is a review: Review + AggregateRating (ratingValue, reviewCount placeholders)\n"
        f"  • BreadcrumbList if applicable\n"
        f"Output schema recommendations as plain text labels — JSON-LD is generated automatically.\n"
        f"═══════════════════════════════════════════════\n"
    )

    # ── v5: Self-check report block ──
    _selfcheck_block = (
        f"═══ ✅ SELF-CHECK REPORT (v5 — append at END of output) ═══\n"
        f"After generating the full article, append a [SELF_CHECK_REPORT] section:\n"
        f"  WORD_COUNT: [count]\n"
        f"  FLESCH_SCORE: [score or N/A for non-EN] | TARGET: >=50\n"
        f"  AVG_SENTENCE_LEN: [words] | TARGET: <=22\n"
        f"  AVG_PARAGRAPH_SENTENCES: [count] | TARGET: <=4\n"
        f"  WALL_OF_TEXT_FLAGS: [yes/no — 5+ paragraphs without heading/list/table]\n"
        f"  KEYWORD_STUFFING: [yes/no]\n"
        f"  HAS_FIRST_HAND_EXPERIENCE: [yes/no — specific numbers/observations]\n"
        f"  HAS_INVENTED_FACTS: [yes/no — if yes list them]\n"
        f"  RESPONSIBLE_GAMBLING_BLOCK: [yes/no]\n"
        f"  ANCHOR_DISTRIBUTION: exact=[%] partial=[%] variation=[%] related=[%] branded=[%] generic=[%] lsi=[%]\n"
        f"  FAQ_FROM_PAA: [yes/no — FAQ built from real PAA questions]\n"
        f"  SCHEMA_TYPES: [list]\n"
        f"═══════════════════════════════════════════════\n"
    )
    ...
    if task_type == "text_mono" and casino:
        return _serp_block + (
            ...
            f"{_paa_block}"
            f"{ANCHOR_DIVERSITY_RULES}"
            f"{_responsible_block}"
            f"{_schema_block}"
            f"{_selfcheck_block}"
            f"{base_rules}"
        )
...
async def get_targets_from_serp(keyword: str, geo: str, client) -> dict:
    ...
    targets = {
        ...
        "paa_questions":  [],  # filled below
    }

    # ── v5: fetch PAA questions in parallel with SERP scraping ──
    try:
        paa = await fetch_paa_questions(keyword, geo, client, limit=20)
        targets["paa_questions"] = paa
        if paa:
            print(f"  💬 PAA: {len(paa)} questions added to targets")
    except Exception as _paa_err:
        print(f"  ⚠️ PAA fetch skipped: {_paa_err}")

    print(f"  ✅ v5 targets computed (median of top-{len(top3)}): "
          f"words={targets['word_count']}, H2={targets['h2_count']}, "
          f"KW density={targets['kw_density'][0]:.1f}-{targets['kw_density'][1]:.1f}%"
          f", PAA={len(targets['paa_questions'])}q")
    return targets
...
async def main():
    ...
    for i, (text, fname) in enumerate(zip(results, filenames)):
        ...
        # v5: стрипуем технические блоки из финального текста
        text = strip_technical_blocks(text)
        ...
        for _orig_attempt in range(ORIG_MAX_RETRIES + 1):
            ...
            if _orig_attempt < ORIG_MAX_RETRIES:
                ...
                if not text:
                    ...
                    break
                # v5: стрипуем технические блоки из регенерированного текста
                text = strip_technical_blocks(text)
