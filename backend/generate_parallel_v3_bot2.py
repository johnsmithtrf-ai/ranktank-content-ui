...
# Telegram bot config
TELEGRAM_BOT_TOKEN = ""  # Disabled — use web UI only  # Disabled — use web UI only  # Disabled
...
def build_prompt(task, article_type, casino=None, text_index=0, serp_targets=None):
    geo = task.get("geo", "UK")
...
async def main():
    ...
    async with httpx.AsyncClient() as client:
        ...
        for i, (atype, casino) in enumerate(prompts):
            prompt = build_prompt(task, atype, casino, text_index=i, serp_targets=serp_targets)
    ...
    for i, (text, fname) in enumerate(zip(results, filenames)):
        ...
        # Originality.ai — background thread (non-blocking)
        import threading as _thr
        _orig_result = {}
        def _run_orig(_txt, _res, _idx):
            import asyncio as _al
            try:
                _loop = _al.new_event_loop()
                _res["orig"] = _loop.run_until_complete(check_originality(_txt))
                _loop.close()
                print(f"  [{_idx+1}] Originality (bg): {_res['orig']['original']}% original")
            except Exception as _e:
                _res["orig"] = {"original": 0, "ai": 100, "ok": False, "note": str(_e)}
        _orig_thread = _thr.Thread(target=_run_orig, args=(text, _orig_result, i), daemon=True)
        _orig_thread.start()
        # (result will appear in logs; join() below after upload to avoid blocking)
        print(f"  [{i+1}] Originality.ai check started in background")
        upload_fname = fname
