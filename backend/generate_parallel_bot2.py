...
TELEGRAM_GROUP_ID = "-5296144980"  # Группа Влада (content2)
...
try:
    ...
    # Override with labeled keys if present
...
# OpenRouter key rotation (more effective than Gemini key rotation)
_or_key_idx = 0
def get_next_or_key():
    global _or_key_idx
...
async def generate_text(client, prompt, idx, task=None):
    ...
    # Alternate between OpenRouter and direct Gemini API for rate limit relief
    DIRECT_GEMINI_URL = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key={GEMINI_KEY_2 or GEMINI_KEY}"
    use_direct = bool(GEMINI_KEY_2) and (idx % 2 == 1)  # odd-indexed texts use direct API

    for attempt in range(3):
        try:
            if use_direct and GEMINI_KEY_2:
                # Direct Google AI Studio API
                direct_payload = {
                    "contents": [{"role": "user", "parts": [{"text": prompt}]}],
                    "generationConfig": {"temperature": 0.7, "maxOutputTokens": 16384}
                }
                r = await client.post(DIRECT_GEMINI_URL, json=direct_payload, timeout=120.0)
                if r.status_code == 200:
                    data = r.json()
                    text_content = data.get("candidates", [{}])[0].get("content", {}).get("parts", [{}])[0].get("text", "")
                    # Wrap in OpenRouter-compatible format for downstream processing
                    data = {"choices": [{"message": {"content": text_content}, "finish_reason": "stop"}]}
                elif r.status_code == 429:
                    use_direct = False  # Fallback to OpenRouter
                    r = await client.post(GEMINI_URL, headers={"Authorization": f"Bearer {OPENROUTER_KEY}", "Content-Type": "application/json"}, json=payload, timeout=120.0)
                else:
                    use_direct = False
                    r = await client.post(GEMINI_URL, headers={"Authorization": f"Bearer {OPENROUTER_KEY}", "Content-Type": "application/json"}, json=payload, timeout=120.0)
            else:
...
def build_prompt(task, article_type, casino=None, text_index=0):
    geo = task.get("geo", "UK")
    ...
    struct_labels = {
        ...
        "faq": f"[SECTION: FAQ] — write H2 in {lang} (min 7 questions, 50-100 words each). Questions and answers ALL in {lang}",
    ...
    base_rules = tf_rule + struct_rule + deep_rule + (
        "═══════════════════════════════════════════\n"
        "MANDATORY SEO & QUALITY RULES\n"
        "═══════════════════════════════════════════\n\n"
        f"▸ WORD COUNT: AT LEAST {word_count} words. End naturally — do not cut mid-thought.\n\n"
        ...
        "  - MINIMUM 4 H2 sections, each 300+ words\n"
        "  - H3 subsections inside major H2 blocks\n"
        "  - Minimum 2 comparison tables (pipe format: col1 | col2 | col3)\n"
        "  - Minimum 2 bullet or numbered lists\n\n"
        "▸ END ORDER (mandatory):\n"
        "  Conclusion (200+ words summarizing key points) →\n"
        "  FAQ LAST: minimum 7 questions, each answer 50–100 words,\n"
    ...
    if task_type == "text_mono" and casino:
        return (
            ...
            f"Meta Title: [SEO title 50-60 chars, include primary keyword + {current_year}]\n"
            f"Meta Description: [description 130-155 chars, include primary keyword, clear value prop]\n"
    ...
    # text_review — multi-brand comparison article
        ...
        return (
            ...
            f"OUTPUT FORMAT — START THE DOCUMENT WITH:\n"
            f"Meta Title: [SEO title 50-60 chars, include primary keyword + {current_year}]\n"
            f"Meta Description: [compelling description 130-155 chars, include primary keyword, clear value prop, NO generic phrases]\n"
            f"(leave one empty line after Meta Description, then start article)\n\n"
...
async def main():
    ...
    if not word_count or word_count is None or (word_count == 4000 and task.get("word_count") is None):
        ...
        if competitors:
            ...
            if word_count is None:
                # Less than 3 competitors accessible — use fallback
    ...
    if task_type == "text_mono":
        ...
        # НЕ делаем отдельные тексты под каждый бренд!
        for i in range(count):
            prompts.append((task_type, None))  # None = используем все бренды из task
    ...
    # Параллельная генерация (true parallel via asyncio.gather — no rate limit on this key)
    print(f"\nЗапускаем {len(prompts)} параллельных запросов к Gemini...")
    
    async with httpx.AsyncClient() as client:
        tasks_gen = []
        for i, (atype, casino) in enumerate(prompts):
            prompt = build_prompt(task, atype, casino, text_index=i)
            tasks_gen.append(generate_text(client, prompt, i, task=task))
    ...
    for i, (text, fname) in enumerate(zip(results, filenames)):
        ...
        # Originality.ai check (informational only - no blocking)
        try:
            orig = await check_originality(text)
            print(f"  [{i+1}] Originality: {orig['original']}% original")
        except Exception as _oe:
            print(f"  [{i+1}] Originality check skipped: {_oe}")
        upload_fname = fname
        
        if dbx_token:
            try:
                dbx_folder = task.get('dropbox_path') or task.get('dropbox_folder', '/sofiya/')
                dbx_path = f"{dbx_folder.rstrip('/')}/{fname}"
