...
async def generate_text(client, prompt, idx, task=None):
    ...
    for attempt in range(3):
        try:
            ...
            # 🚨 H1 CHECK — REJECT if missing and retry
            import re as _reH1
            # Support: 'H1: Title', '# Title' (markdown), '<h1>Title</h1>' (html)
            has_h1 = (
...
def build_prompt(task, article_type, casino=None, text_index=0):
    ...
    base_rules = tf_rule + struct_rule + deep_rule + aggressive_rule + (
        ...
        "═══════════════════════════════════════════\n\n"
        "▸ WORD COUNT: Целевой объём текста: 3500-4000 слов. Закончи текст естественно, не обрывай мысль.\n\n"
